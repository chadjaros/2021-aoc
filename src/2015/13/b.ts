...
function search(node: Node, path: Node[], seen: Set<string>, graph: Map<string, Node>): number {
    if (seen.has(node.id)) {
        return -Infinity;
    }
    ...
    if (node.edges.every((x) => seen.has(x.nodeId))) {
        ...
        return path.reduce((acc, node, idx) => {
            ...
            const leftWeight = node.edges.find((e) => e.nodeId === path[leftIdx].id)?.weight;
            const rightIdx = idx + 1 >= path.length ? 0 : idx + 1;
            const rightWeight = node.edges.find((e) => e.nodeId === path[rightIdx].id)?.weight;

            return acc + leftWeight! + rightWeight!;
        }, 0);
    }

    return Math.max(...node.edges.map((e) => {
        const edge = graph.get(e.nodeId)!;
        return search(edge, [...path, node], new Set([...seen.values(), node.id]), graph);
    }));
}
