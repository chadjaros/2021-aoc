...
export class Vector3 extends Matrix {
    ...
    unit(): Vector3 {
        ...
        return Vector3.fromCoordinates(
            this.x * mag,
            this.y * mag,
            this.z * mag
        );
...
export class BoundingBox3 {
    ...
    intersection(bb: BoundingBox3): Possible<BoundingBox3> {
        ...
        const min = Vector3.fromCoordinates(
            Math.max(this.min.x, bb.min.x),
            Math.max(this.min.y, bb.min.y),
            Math.max(this.min.z, bb.min.z)
        );
        const max = Vector3.fromCoordinates(
            Math.min(this.max.x, bb.max.x),
            Math.min(this.max.y, bb.max.y),
            Math.min(this.max.z, bb.max.z)
        );
