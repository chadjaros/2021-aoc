...
const a = new Room('A', false, 4, ['D', 'D', 'D', 'C']);
const b = new Room('B', false, 4, ['D', 'C', 'B', 'C']);
const c = new Room('C', false, 4, ['A', 'B', 'A', 'B']);
const d = new Room('D', false, 4, ['A', 'A', 'C', 'B']);
...
const target = new BoardState(
    new Map([left, h1, h2, h3, right, a.clone(['A', 'A', 'A', 'A']), b.clone(['B', 'B', 'B', 'B']), c.clone(['C', 'C', 'C', 'C']), d.clone(['D', 'D', 'D', 'D'])].map((x) => [x.id, x])),
    weights,
    distances,
);
