...
import { BTree } from '../../ts-utils/b-tree';
...
export class SNNumber {
    constructor(public tree: SNBTree) { }

    plus(num: SNNumber): SNNumber {
        return new SNNumber(
            parseSNTuple(
                this.toTuple(
                    new BTree(undefined, { left: this.tree, right: num.tree })
                )
            )
        ).reduce();
    }

    private replaceParentWith(
        parent: SNBTree,
        oldChild: SNBTree,
        newChild: SNBTree
    ): void {
    ...
    private doExplode(current: SNBTree): boolean {
        if (current.parent && current.parent.depth === 4) {
            ...
            this.replaceParentWith(
                exp.parent!,
                exp,
                new BTree(0, { parent: exp.parent })
            );
            return true;
        }
        return false;
    ...
    private doSplit(current: SNBTree): boolean {
        if ((current.value ?? 0) >= 10) {
            ...
            replace.left = new BTree(Math.floor(current.value! / 2), {
                parent: replace,
            });
            replace.right = new BTree(Math.ceil(current.value! / 2), {
                parent: replace,
            });
