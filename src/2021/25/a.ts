...
while(moved) {
    ...
    const replaceRight: {old: string, new: {point: Point, value:string}}[] = [];
    
    rights.forEach((x) => {
        const n = next(x);
        const nKey = n.key;
        if (!rights.has(nKey) && !downs.has(nKey)) {
            replaceRight.push({old: x.point.key, new: {point: n, value: x.value}});
        }
    });
    ...
    downs.forEach((x) => {
        ...
        const nKey = n.key;
        if (!rights.has(nKey) && !downs.has(nKey)) {
            replaceDown.push({old: x.point.key, new: {point: n, value: x.value}});
        }
    });
