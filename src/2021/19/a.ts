import { Vector3 } from '../../ts-utils/point-3d';
import { seriesSum } from '../../ts-utils/series-math';
...
for (let i = 1; i < order.length; i++) {
    ...
    const pair = pairs.find(
        (x) =>
            (x.a.id === target.id && complete.has(x.b.id)) ||
            (x.b.id === target.id && complete.has(x.a.id))
    );
    ...
    const sourceMatches = (
        pair.a === target ? pair.bMatches : pair.aMatches
    ).map((x) => {
        return {
    ...
    const rotation = rotationMatrices.find((rm) => {
        const rotated = targetDifferences.map((d) => ({
        ...
        sourceMatches.forEach((d) => {
            rotated.forEach((r) => {
                if (d.difference.equals(r.difference)) {
                    ...
                    translations.set(t.toString(), {
                        t,
                        hits: 1 + (translations.get(t.toString())?.hits ?? 0),
                    });
        ...
        if (hits >= matchesFor12Points) {
            ...
            newScanner.translation = [...translations.values()]
                .reduce((max, t) => {
                    if (t.hits > max.hits) {
                        return t;
                    }
    ...
    console.log(
        target.id,
        rotation?.toString(),
        newScanner.translation?.toString()
    );
    if (!rotation) {
    ...
    newScanner.beacons = target.beacons.map((b) =>
        Vector3.fromMatrix(rotation.dot(b).plus(newScanner.translation))
    );
