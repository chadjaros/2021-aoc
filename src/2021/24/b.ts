...
const dInstructions = input24.reduce<{
    digit: number;
    values: ALUInstruction[][];
}>(
    (accum, val) => {
        if (val[0] === 'inp') {
            accum.digit++;
...
const stack: number[] = [];
const operations = dInstructions.values
    .reduce<Pair[]>((accum, x, i) => {
        if (x[4][2] === 1) {
            stack.push(i);
            return accum;
        } else {
...
function findPairForIndex(d: number, e: number): Pair {
    for (const f of Series.range(1, 9)) {
        for (const g of Series.range(1, 9)) {
            const a = new ALU();
...
const result = operations.reduce<number[]>(
    (accum, x) => {
