...
function diriacFork(players: Player[], playerTurn: number, [roll, count]: number[]): Result {
...
function diriacRound(players: Player[], playerTurn: number, count: number): Result {

    const playerWins = [0, 0];
    for (const d of rollDistribution) {
        const result = diriacFork(players.map((p) => p.clone()), playerTurn, d);
