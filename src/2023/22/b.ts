...
aoc((infile) => {
    ...
    const value = ArrayUtils.tail(stack).reduce((acc, box) => {

        const gone = new Set<string>([box.bbox.key]);
        const localStack = [...box.supports];

        while (localStack.length !== 0) {
            const key = localStack.shift()!;
            if (gone.has(key)) {
                continue;
            }
            const current = stackMap.get(key)!;

            if (current.supportedBy.every((_) => gone.has(_))) {
                gone.add(key);
                localStack.push(...current.supports);
            }
        }

        return acc + gone.size - 1;
    }, 0);
