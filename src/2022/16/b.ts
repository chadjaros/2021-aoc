...
import { difference } from '../../utils/set-math';
...
aoc((infile) => {
    ...
    const withFlow = locationGraph.allNodes.filter((x) => x.flow > 0).sort((a, b) => b.flow - a.flow);

    const locationPrecompute = new Map<string, Map<string, number>>();
    locationGraph.allNodes.filter((x) => x.id === start.id || x.flow > 0).forEach((l) => {
        locationPrecompute.set(l.id, new Map(Array.from(dijkstra(l, locationGraph)!.distances.entries()).filter(([id]) => locationGraph.getNode(id).flow > 0)));
    });
    ...
    function dfs(
        ...
        let newFlow = flow;
        const newLog = [...log];
        const acted = new Map([Me, Ele].map((m) => [m, (actions.get(m)?.completesAt ?? time) < time]));

        newLog.push(`--- m${27-time} --- open: ${Array.from(difference(flowSet, unopened).values()).join(',')} flow ${flow} total ${total}`);
        ...
        function processOpen(person: string): void {
            ...
            if (location.flow !== 0 && unopened.has(location.id)) {
                ...
                newLog.push(`t${time} ${person} open ${location.id} flow +${location.flow}`);
        ...
        if (toMove.length > 0) {
            ...
            const possibles = Array.from(available.values())
                .map((x) => ({id: x, distance: locationPrecompute.get(location.id)?.get(x) ?? time}))
                .filter((x) => (x.distance + 1) < time);

            if (possibles.length > 0) {

                const specialCaseAvailable = new Set(available);
                const isSpecialCase = locations.get(Me)?.id === locations.get(Ele)?.id;
                if(isSpecialCase) {
                    console.log('specialCase', time, locations.get(Me)?.id); 
                }


                return possibles.reduce((acc, value, idx) => {
                    ...
                    newLocations.set(person, locationGraph.getNode(value.id));
                    newActions.set(person, {
                        action: 'move',
                        completesAt: time - value.distance,
                    });
                    const moveLog = [...newLog, `t${time} ${person} move ${value.distance} to ${value.id}`];
                    ...
                    if (toMove.length === 2) {
                        ...
                        const poss2Src = isSpecialCase ? specialCaseAvailable : newAvailable;

                        if (isSpecialCase) {
                            console.log('reduce', Array.from(newAvailable.values()).join(','), 'to', Array.from(specialCaseAvailable.values()).join(','));
                        }

                        const possibles2 = Array.from(poss2Src.values())
                            .map((x) => ({id: x, distance: locationPrecompute.get(location2.id)?.get(x) ?? time}))
                            .filter((x) => (x.distance + 1) < time);

                        if (possibles2.length > 0) {
                            const result = possibles2.reduce<[number, string[]]>((acc2, value2) => {
                                const newAvailable2 = new Set(newAvailable);
                                const newActions2 = new Map(newActions);
                                const newLocations2 = new Map(newLocations);
                                newAvailable2.delete(value2.id);
                                newLocations2.set(person2, locationGraph.getNode(value2.id));
                                ...
                                const result2 = dfs(
                                    ...
                                    newFlow, 
                                    total + flow, 
                                    [...moveLog, `t${time} ${person2} move ${value2.distance} to ${value2.id}`]
                                );
                                
                                return result2[0] > acc2[0] ? result2 : acc2;
                            }, [-Infinity, ['start']]);
