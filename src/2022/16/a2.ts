...
aoc((infile) => {
    ...
    const withFlow = locationGraph.allNodes.filter((x) => x.id === location.id || x.flow > 0);
    ...
    function dfs(
        ...
        if (location.flow !== 0 && unopened.has(location.id)) {
            ...
            return dfs(
                ...
                flow + location.flow, 
                flow + total, 
                [...log, `t${time} open ${location.id} flow +${location.flow} flow ${flow} total ${total}`]
            );
        }
        ...
        if (possibles.length > 0) {
            return possibles.reduce((acc, value) => {
                const result = dfs(
                    ...
                    [...log, `t${time} move ${value.distance} to ${value.id} flow ${flow} total ${total}`]
                );
        ...
        return dfs(location, unopened, 0, flow, total + (flow * time), [...log, `t${time} wait until end flow ${flow} total ${total}`]);
    }

    const result = dfs(
        location,
        new Set(withFlow.map((x) => x.id)),
        30,
        0,
        0,
