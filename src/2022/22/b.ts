...
aoc((infile) => {
    ...
    if (faceSize === 4) {
        ...
        transition = (p: Point, dir: Direction) => {
            ...
            // Top top
            if (gridY < 0) {
                result = [new Point(reverse(p.x - 2 * faceSize), faceSize), down];
            }
            ...
            // Top right
                ...
                result = [new Point(faceSize * 4 - 1, reverse(p.y) + 2*faceSize), left];
            }
            else if (gridX === 4) {
                result = [new Point(faceSize * 3 - 1, reverse(p.y - 2*faceSize)), left];
            }
            // far left
            else if (gridX < 0) {
                result = [new Point(3*faceSize + reverse(p.y - faceSize), faceSize * 3 - 1), up];
            }
            else if (gridY === 3 && gridX === 3) {
                result = [new Point(0, faceSize + reverse(p.x - 3*faceSize)), right];
            }
            // Bottom left
            else if (gridY === 2 && gridX === 1) {
                if (dir === down) {
                    result = [new Point(2*faceSize, reverse(p.x - faceSize) + 2*faceSize), right];
                }
                else if (dir === left) {
                    result = [new Point(reverse(p.y - 2*faceSize) + faceSize, 2*faceSize - 1), up];
                }
                else {
                    console.log('fail', p.key, dir.name);
                    throw Error();
            ...
            // Middle right
                ...
                if (dir === right) {
                    result = [new Point(reverse(p.y - faceSize) + 3 * faceSize, 2*faceSize), down];
                }
                else if (dir === up) {
                    result = [new Point(3 * faceSize - 1, reverse(p.x - 3 * faceSize) + faceSize), left];
                }
                else {
                    console.log('fail', p.key, dir.name);
                    throw Error();
            ...
            // bottom
                ...
                result = [new Point(reverse(p.x - 2*faceSize), 2*faceSize - 1), up];
            }
            else if (gridX === 0 && gridY === 2) {
                result = [new Point(reverse(p.x) + 2*faceSize, 3*faceSize - 1), up];
            }
            else {
                console.log('fail', p.key, dir.name);
                throw Error();
        ...
        transition = (p: Point, dir: Direction) => {
            ...
            // 3-7, 7-3
                ...
                result = [new Point(faceSize, reverse(p.y - 2*faceSize)), right];
            }
            // 4-8, 8-4
            else if (gridX === 3) {
                result = [new Point(2*faceSize - 1, reverse(p.y) + 2 * faceSize), left];
            }
            else if (gridX === 2 && gridY === 2)
            {
                result = [new Point(3*faceSize - 1, reverse(p.y - 2 * faceSize)), left];
            }
            // 5
    ...
    function move(start: Point, direction: Direction, count: number): [Point, Direction] {

        let current = start;
        let dir = direction;
        for (let mv = 0; mv < count; mv++) {
            let next = new Point(current.x + dir.vector.x, current.y + dir.vector.y);
